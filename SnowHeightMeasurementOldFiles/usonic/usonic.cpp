	...
	_distance(0)
{
	...
	// Pin Definitions
	usonicDir |= (1 << usonicTriggerPin);
	usonicPort &= ~(1 << usonicTriggerPin);
	
	// init Timer
	usonicTimsk |= (1 << usonicTOVF);
	usonicTriggerImpuls = 180;
	
	// init Ext. Interrupts
	usonicEchoEICR |= (1 << usonicInterrupt); // rising and falling edge sense
...
uint16_t usonic::getAverageDistance()
{
	...
	{			
		...
		startMeasurement();		
		//USART_sendBytes("before delay ");	
		while(!_ready) {
			_delay_us(0);
		}
		//USART_sendBytes("after delay ");
	...
	//DEBUG START
	//USART_sendBytes("\n");
	//USART_sendBytes(" Unsorted Values: \0");
	//for (uint8_t i = 0; i < 10; ++i)
	//{
		//USART_sendInteger(measureDistances[i]);
		//USART_sendBytes(" \0");
	//}
	//USART_sendBytes("\n");
	//DEBUG END
	...
	//DEBUG START
	//USART_sendBytes(" Sorted Values: \0");
	//for (uint8_t i = 0; i < 10; ++i)
	//{
		//USART_sendInteger(measureDistances[i]);
		//USART_sendBytes(" \0");
	...
	//DEBUG START
	//USART_sendBytes(" Compare: \0");
	//USART_sendInteger(compareValue);
//
	//USART_sendBytes(" Low: \-0");
	//USART_sendInteger(lowIndex);
//
	//USART_sendBytes(" High: \0");
	//USART_sendInteger(highIndex);
//
	//USART_sendBytes("\n");
	//USART_sendBytes(" Accepted Values: \0");
	...
	//USART_sendBytes(" Value: \0");
	//USART_sendInteger(averageValue / (highIndex - lowIndex));
//
	//USART_sendBytes(" Values: \0");
	//for (uint8_t i = lowIndex; i < highIndex; ++i)
	//{
		//USART_sendInteger(measureDistances[i]);
		//USART_sendBytes(" \0");
